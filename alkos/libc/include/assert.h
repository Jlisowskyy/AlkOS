...
#else
#define ASSERT(expr) if (!(expr)) { __FAIL_KERNEL(expr); }
#endif // NDEBUG

/* usual kernel working in release assert macro */
#define R_ASSERT(expr) if (!(expr)) { __FAIL_KERNEL(expr); }
...
#define ASSERT_EQ(expected, value)  BASE_ASSERT_EQ(kIsDebugBuild, expected, value, __ASSERT_FAIL_FUNC)
#define ASSERT_NEQ(expected, value) BASE_ASSERT_NEQ(kIsDebugBuild, expected, value, __ASSERT_FAIL_FUNC)
